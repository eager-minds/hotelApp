...
// This get will throw an exception if the service is not running
  ...
  public void request_body_is(${topicEntityName}ValueDto ${topicEntityNameLowerCamelCase}ValueDto) {
    scenarioContext.setSaved${topicEntityName}ValueDto(${topicEntityNameLowerCamelCase}ValueDto);
    requestSpecification.body(${topicEntityNameLowerCamelCase}ValueDto);
  }
  ...
  @When("^I get body from response of ${topicEntityName}KeyDto type$")
  public void get_body_from_response_of_${topicEntityNameLowerCamelCase}Key_type() throws JsonProcessingException {
    scenarioContext.setSaved${topicEntityName}KeyDto(
        new ObjectMapper().readValue(scenarioContext.response().body().asString(), ${topicEntityName}KeyDto.class));
  }
