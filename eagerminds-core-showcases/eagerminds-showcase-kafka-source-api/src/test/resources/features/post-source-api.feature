Feature: Service to [send kafka message]
  ...
  Scenario: [Send a kafka message from received rest API body]
    Given kafka has a topic "eagerminds-showcase"
    And I have a kafka consumer service
    And content type is "JSON"
    And request body is
"""eagerMindsShowcase_value_dto_json
{
  "myString": "eagerMinds",
  "myInt": 3,
  "myIntArray": [1, 2, 3, 4]
...
"""
    Then POST /kafka-eagerMindsShowcase/send
    Then I get response HTTP code 201 CREATED
    And I get body from response of EagerMindsShowcaseKeyDto type
